...
# customer_frame = LabelFrame(root,text="Customer Details",font=("arial", 15, "bold"),bg="#5F755E", fg = '#F3EAD4',relief=GROOVE)
...
customer_name_label = Label(f1, text="Name", font=("arial", 15, "bold"),bg = "#5F755E", fg="#F3EAD4")
customer_name_label.pack(side=TOP)

customer_name_entry = Entry(f1,width=20,font="arial 15",bd=5,textvariable=customerName)
customer_name_entry.pack(side = TOP)

customer_contact_label = Label(f1, text="Contact", font=("arial", 15, "bold"),bg = "#5F755E", fg="#F3EAD4")
customer_contact_label.pack(side =TOP)

customer_contact_entry = Entry(f1,width=20,font="arial 15",bd=5,textvariable=customerContact)
customer_contact_entry.pack(side=TOP)
...
menulist=["Cappuccino","Americano","Cold_Brew","CC Muffin","Strawberry Smoothie","Straberry Oreo","Cr Oreo Fr",\
          "Double Chocolate Fr","Season Special 1","Season Special 2","ColdBrewFV","CheeseCake","Hot Chocolate",\
              "Waffels","MUDPie Shake","ButterBeer","MochaFrappe","Latte","Menu Of The day 1","Menu Of The day 2"]
pricelist=[125,120,110,70,140,150,160,155,120,85,90,150,145,180,135,125,140,140,150,150]
bilist=[]
billdf = pd.DataFrame(index=menulist)
x1=random.randint(10908, 500876)
...
bi ="E:/Cafe Management/bill records/"
...
def CostofItems():
    if (len(customerName.get()) == 0):
        messagebox.showerror("showerror", "Error: Invalid Customer details")
        customerName.set("")
    elif (len(customerContact.get()) == 0):
        messagebox.showerror("showerror", "Error: Invalid Customer Details")
        customerContact.set("")
    ...
    binm = str(customerName.get())       #         add to sql table
    bilc = str(customerContact.get())    #            add to sql table
    b = ""
    bv = str(Receipt_Ref.get())
    if bv==b:
        bv=str(x1)
    b1 = """INSERT INTO custdet VALUES(%s,%s,%s)"""
    b2 = (binm,bilc,bv)
    mycursor.execute(b1,b2)
    mydb.commit()
    #  enter into table binm,bilc,bv
    b = bi + bv + ".csv"    
    orderlist = [Item1,Item2,Item3,Item4,Item5,Item6,Item7,Item8,Item9,Item10,Item11,Item12,Item13,Item14,Item15,Item16,Item17,Item18,Item19,Item20]
    ST=0
    for i in range(20):
        for j in range(20):
            if i==j:
                bilist.append(pricelist[i]*orderlist[j])
                ST+= (pricelist[i]*orderlist[j])
    billdf["Quantity"]=orderlist
    billdf["Rate"]=pricelist
    billdf["IndividualCost"]=bilist
    billdf.to_csv(b)
    ...
    txtReceipt.insert(END,'Receipt Ref:\t\t\t'+ bv + '\t\t'+ DateofOrder.get()+"\n")
...
def chkbutton_value():
    ...
    if (var3.get() == 1):
        txtCold_Brew.configure(state=NORMAL)
            ...
            E_Cold_Brew.set("0")
    if (var4.get() == 1):
        txtCCMff.configure(state=NORMAL)
    elif var4.get()== 0:
            txtCCMff.configure(state=DISABLED)
            E_CCMff.set("0")
    if (var5.get() == 1):
...
#================================================= placing menu item check buttons ===================================================================================================================
...
txtCCMff = Entry(f1aa,font=('arial',16,'bold'),bd=8,width=6,justify='left',textvariable=E_CCMff,state=DISABLED)
txtCCMff.grid(row=3,column=1)
...
txtCold_Brew_FV = Entry(f1ab,font=('arial',16,'bold'),bd=8,width=6,justify='left',textvariable=E_Cold_Brew_FV,state=DISABLED)
txtCold_Brew_FV.grid(row=0,column=1)
...
#======================================================================Search============================================================================
def srchb():
    execfile("E:\Cafe Management\sf1.py")
...
btnSearch=Button(fb2,padx=16,pady=1,bd=4,fg="black",font=('arial',16,'bold'),width=5,text="Search",command=srchb).grid(row=0,column=3)

btnExit=Button(fb2,padx=16,pady=1,bd=4,fg="black",font=('arial',16,'bold'),width=5,text="Exit",command=goback).grid(row=0,column=4)
