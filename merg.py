# -*- coding: utf-8 -*-
"""
Created on Wed Jan  5 18:00:10 2022
...
def CostofItems():
    ...
    billdf.to_csv("D:/hs.csv")

    SubTotalofITEMS = "Rs" , str('%.2f'%(ST))
    SubTotal.set(SubTotalofITEMS)

    Tax= "Rs" , str('%.2f'%((ST)* 0.15))
    PaidTax.set(Tax)
    ...
    while a3 > 0:        
        ...
        break
    while a4 > 0:     
        txtReceipt.insert(END,'Chocochip Muffin:\t\t\t'+ E_CCMff.get()+"\n")
        break
    while a5 > 0:      
        txtReceipt.insert(END,'Strawberry Smoothie:\t\t\t'+ E_Sr_Smoothie.get()+"\n")
        break
    while a6 > 0:
        txtReceipt.insert(END,'Sr oreo Frappe:\t\t\t' + E_SOF.get()+"\n")
        break
    while a7 > 0:
        txtReceipt.insert(END,'Cr Oreo Frappe:\t\t\t' + E_COF.get()+"\n")
        break
    while a8 > 0:        
        txtReceipt.insert(END,'Double Ch Fr:\t\t\t' + E_DCF.get()+"\n")
        break
    while a9 > 0:
        txtReceipt.insert(END,'ColdBrewFV:\t\t\t' + E_Cold_Brew_FV.get()+"\n")
        break
    while a10 > 0:
...
btnReceipt=Button(fb2,padx=16,pady=1,bd=4,fg="black",font=('arial',16,'bold'),width=5,text="Receipt",command=CostofItems).grid(row=0,column=1)
